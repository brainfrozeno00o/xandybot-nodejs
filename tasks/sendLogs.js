    ...
    async execute(client) {
        try {
            cron.schedule("*/5 * * * *", async function() {
                ...
                // set up initial date
                const dateString = new Date().toLocaleString("en-SG", { timeZone: "Asia/Singapore" });
                const firstPart = "```Log at this time: " + dateString;
                // get all guilds
                const allGuilds = await client.guilds.cache.map(guild => guild.name);
                const guildCountString = "Number of servers currently serving: " + allGuilds.length;
                const allGuildString = allGuilds.join("\n");
                const serverString = "Server List:\n" + allGuildString;
                // TODO: Get info about quotes (can be removed)
                // set up message here
                const logMessage = [firstPart, guildCountString, serverString].join("\n\n") + "```";
                // get message
                const logChannel = await client.channels.fetch(BigInt(logChannelId));
                const messageId = logMessageId === "" ? 0 : logMessageId;
                const message = messageId === 0 ? null : await logChannel.messages.fetch(messageId);
            ...
            console.error(`Error occurred while setting up task for sending logs: ${e}`);
        }
    }
};